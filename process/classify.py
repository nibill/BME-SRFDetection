...
def classify(image_struc) :
    ...
    hough = process.image.hough(lineSegment.copy(),clean.copy())

    count = process.image.count(hough.copy())

    result = ""

    return [image_struc[2],image_struc[0],result,count,image,openi,colorMap,reduced,grabCut,newMask,newImage,reduced2,clean,lineSegment,hough]